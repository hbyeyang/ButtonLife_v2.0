 ...
 */
public class ModifyAddress extends BaseActivity implements OnWheelChangedListener {
    private EditText et_name, et_phone, et_address, et_district;
    private MyAddress myAddress;
    private PopupWindow popupWindow;
    private TextView titleBarTv, confirm, cancel;
    private WheelView mViewDistrict, mViewCity, mViewProvince;
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.title_bar_left) {//点击返回
            ...
            intent.putExtra(UserLogic.EXTRA_USER_DISTRICT, mCurrentDistrictName);
            sendAction(intent);
            finish();
    ...
    protected void init(Bundle saveInstanceState) {
        ...
        myAddress = (MyAddress) intent.getExtras().getSerializable(UserLogic.EXTRA_ADDRESS_ID);

        et_name.setText(myAddress.getContactName());
        et_phone.setText(myAddress.getContactTel());
        et_address.setText(myAddress.getConsignAddress());

        if (!TextUtils.isEmpty(myAddress.getConsignDistrict())) {
            mCurrentDistrictName = myAddress.getConsignDistrict();
            et_district.setText(mCurrentDistrictName);
        }
    }
    ...
    private void showPopWindow() {
        if (popupWindow == null) {
            ...
            popupWindow.update();
            popupWindow.showAtLocation(findViewById(R.id.rl_modify_address), Gravity.BOTTOM, 0, 0);
        } else {
            popupWindow.showAtLocation(findViewById(R.id.rl_modify_address), Gravity.BOTTOM, 0, 0);
        }
    }
